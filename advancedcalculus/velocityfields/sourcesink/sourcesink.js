...
let ymax = 4;
let ymin = -4;
let sc = 0.2;// vector field size
let xstep = 0.6;//vector density
let ystep = 0.6;
...
function draw() {
    ...
    if(starting==true) {
        ...
        for (let i=particles.length-1; i>=0; i-=1) {
            ...
            if ( p.x > bx ||  p.y > by || p.x < -bx ||  p.y < -by ) {
                ...
                currentParticle--;
                particles.push(new Particle(random(-bx,bx),random(-by,by),t,h) );
            }
        }
        //}
    ...
    text('k= '+nfc(sliderk.value(),1,1),-2.6,14);//for slider k
...
function componentFX(t, x, y){
    return ( sliderk.value()*x/(x*x+y*y) );//Change this function
}

function componentFY(t, x, y){
    return (  sliderk.value()*y/(x*x+y*y) );//Change this function
}
